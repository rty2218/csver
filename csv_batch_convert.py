...
def main() -> int:
    ...
    for csv_path in csv_files:
        try:
            rows, encoding, delimiter = read_csv_rows(csv_path, args.encoding, args.delimiter)
            ...
            write_xlsx(rows, xlsx_path, sheet_name=base_name, infer_types=args.infer_types)
            write_txt_table(rows, txt_path, max_col_width=args.max_col_width)
            write_markdown_table(rows, md_path, no_header=args.no_header, infer_align=args.infer_types)
...
def read_csv_rows(path: Path, encoding: str, delimiter: str) -> tuple[list[list[str]], str, str]:
    ...
    for candidate in encodings:
        try:
            text = path.read_text(encoding=candidate, newline="")
...
def infer_xlsx_value(value: str) -> tuple[str, str] | None:
